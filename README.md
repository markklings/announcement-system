...
Below is the Use Case Diagram for the **Classroom Announcement System**, depicting actor interactions and permission workflows.

![Classroom Announcement System Use Case Diagram](./Use_Case_Diagram.png)

### Actors & Core Responsibilities

| Actor | Description & Actions |
| :--- | :--- |
| **Classroom Officer / Teacher** | Manages class students, announcements, storage/contents, and requests access to the Admin Dashboard. |
| **Super Admin** | Authorizes teacher access, manages active accounts, monitors system logs, and manages system-wide announcements. |
| **Students** | Accesses the student dashboard, views announcements, and asks questions on announcements. |

### Key Workflow Rules
* **Authorization Extension (`<<extend>>`):** `Authorize Admin Access` extends the `Request Admin Dashboard` use case conditionally when a teacher requires permission from the Super Admin.
---
